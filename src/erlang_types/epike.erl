...
is_satisfiable(Constraints, Fixed) ->
  ...
  case all_of(Goals, #{}, #{}, fun(_C, _X) -> true end, #{}) of
    true -> true;
    {false, _Reason} -> false
  end.
      ...
      case intersect_bound(U, CU, Env) of
        ...
        U1 ->
          ...
          case CL of
            Empty -> K(C1, X);
            _ ->
              empty(ty_node:difference(CL, U1), C1, X, K, maps:merge(RL, maps:merge(RU, Path)), Env)
          end
      end;
    _ ->
      ...
      case union_bound(L, CL, Env) of
        ...
        L1 ->
          ...
          case CU of
            Any -> K(C1, X);
            _ ->
              empty(ty_node:difference(L1, CU), C1, X, K, maps:merge(RU, maps:merge(RL, Path)), Env)
          end
      end;
    _ ->
      K(C#{V => {L, Any, Path, #{}}}, X)
...
explore(T1, T2, [F | Ps], C, X, K, Path, Env) ->
