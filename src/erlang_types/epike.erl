...
is_satisfiable(Constraints, Fixed) ->
  ...
  all_of(Goals, #{}, #{}, fun(_C, _X) -> true end).
...
any_of([G | Gs], C, X, K) ->
  G(C, X, K) orelse any_of(Gs, C, X, K).
      ...
      case intersect_bound(U, CU, Env) of
        ...
        U1 ->
          ...
          case CL of
            Empty -> K(C1, X);
            _ ->
              empty(ty_node:difference(CL, U1), C1, X, K, Env)
          end
      end;
    _ ->
      K(C#{V => {Empty, U}}, X)
      ...
      case union_bound(L, CL, Env) of
        ...
        L1 ->
          ...
          case CU of
            Any -> K(C1, X);
            _ ->
              empty(ty_node:difference(L1, CU), C1, X, K, Env)
          end
      end;
    _ ->
      K(C#{V => {L, Any}}, X)
...
phi(BigS, Neg, C, X, K, Env) ->
  ...
  case Neg of
    [] -> any_of(Components, C, X, K);
    [Ty | N] ->
      any_of(Components, C, X, K)
        orelse all_of(without(BigS, ty_tuple:components(Ty), 1, N, Env), C, X, K)
  end.
...
function_cont(S, P, [F | N], C, X, K, Env) ->
  ...
  all_of([empty_goal(ty_node:intersect(T1, ty_node:negate(S)), Env),
    ...
    orelse function_cont(S, P, N, C, X, K, Env).
