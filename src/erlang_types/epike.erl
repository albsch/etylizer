...
is_satisfiable(Constraints, Fixed) ->
  ...
  case all_of(inputs, Goals, S0, fun(_S) -> true end, #{}) of
    true -> true;
...
empty(T, S = #s{reads = Reads, learned = Learned}, K, Path, Env) ->
  case {ty_node:empty(), ty_node:any()} of
    {T, _} -> K(S);
    {_, T} -> {false, Path, Reads, Learned};
    _ -> empty_node(T, S, K, Path, Env)
  end.

-spec empty_node(ty:type(), s(), k(), reason(), env()) -> result().
empty_node(T, S = #s{c = C, x = X, epoch = E0, reads = Reads0, tok = Tok0, learned = Learned0}, K, Path, Env = #env{fixed = Fixed}) ->
  case X of
    #{{node, T} := _} -> K(S);
    _ ->
